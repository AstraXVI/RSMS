 ...
 if (isset($_GET['search'])) {
     # code...
    $JOBPOSITION = $_GET['search'];
 }else{
     $JOBPOSITION = '';

 }
    $sql = "SELECT j.JOBID, j.OCCUPATIONTITLE, c.COMPANYID, c.COMPANYNAME, c.COMPANYADDRESS, j.DATEPOSTED 
    FROM `tbljob` j INNER JOIN `tblcompany` c ON j.COMPANYID = c.COMPANYID 
    WHERE j.CATEGORY = '$JOBPOSITION'" ;
    $mydb->setQuery($sql);
    ...
    foreach ($cur as $result) {
        ...
        echo '<td><a href="'.web_root.'index.php?q=viewjob&search='.$result->COMPANYID.'&jobposition='.$JOBPOSITION.'">'.$result->COMPANYNAME.'</td>';
